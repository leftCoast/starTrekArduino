...
static int tryexit(int lookx, int looky, int ienm, int loccom, int irun) {
	...
	if (damage[DSRSENS] == 0.0 || damage[DLRSENS] == 0.0 ||
		...
		proutn((char*)"***");
		cramen(ienm);
		proutn((char*)" escapes to");
		cramlc(1, iqx, iqy);
		prout((char*)" (and regains strength).");
	}
  ...
  */
	...
	if (nextx != comx || nexty != comy) {
		...
		if (damage[DSRSENS] == 0 || condit == IHDOCKED) {
			proutn((char*)"***");
			cramen(ienm);
			if (kdist[loccom] < dist1) proutn((char*)" advances to");
			else proutn((char*)" retreats to");
	...
	for (i = 1; i <= inplan; i++) {
		if (d.plnets[i].x==d.isx && d.plnets[i].y==d.isy &&
			...
			if (REPORTS) {
				if (*ipage==0) pause(1);
				*ipage = 1;
				prout((char*)"Lt. Uhura-  \"Captain, Starfleet Intelligence reports");
				proutn((char*)"   a planet in");
				cramlc(1, d.isx, d.isy);
				prout((char*)" has been destroyed");
				prout((char*)"   by the Super-commander.\"");
			}
			break;
		}
	}
...
void scom(int *ipage) {
	...
	if (d.rembase == 0) {
		...
		if (ibqx==d.isx && ibqy == d.isy && d.isx != batx && d.isy != baty) {
			...
			*ipage=1;
			...
			cramlc(1, d.isx, d.isy);
			skip(1);
			prout((char*)"   reports that it is under attack from the Klingon Super-commander.");
			proutn((char*)"   It can survive until stardate ");
			cramf(future[FSCDBAS], 0, 1);
			prout((char*)" .\"");
			if (resting==0) return;
			prout((char*)"Mr. Spock-  \"Captain, shall we cancel the rest period?\"");
			if (ja()==0) return;
			resting = 0;
	...
	*ipage = 1;
	prout((char*)"Lt. Uhura-  \"Captain, Starfleet Intelligence reports");
	proutn((char*)"   the Super-commander is in");
	cramlc(1, d.isx, d.isy);
	prout((char*)".\"");
	return;
}
