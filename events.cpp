...
void events(void) {
	...
	for (;;) {
		...
		if (stdamtim != 1e30 && REPORTS) {
			stdamtim = 1e30;
			prout((char*)"Lt. Uhura- \"Captain, the sub-space radio is working and");
			prout((char*)"   surveillance reports are coming in.");
			skip(1);
			for (i=1; i <= 8 ; i++)
				for (j=1; j <= 8; j++)
			...
			skip(1);
			prout((char*)"   The star chart is now up to date.\"");
			skip(1);
		}
		...
		switch (line) {
			...
			case FTBEAM: /* Tractor beam */
				...
				proutn((char*)"***");
				crmshp();
				prout((char*)" caught in long range tractor beam--");
				...
				if (iscraft==0) {
					skip(1);
					if (Rand() >0.5) {
						prout((char*)"Galileo, left on the planet surface, is captured");
						prout((char*)"by aliens and made into a flying McDonald's.");
						damage[DSHUTTL] = -10;
						iscraft = -1;
					}
					else {
						prout((char*)"Galileo, left on the planet surface, is well hidden.");
					}
				}
				if (line==0) {
					quadx = d.isx;
				...
				iran10(&sectx, &secty);
				crmshp();
				proutn((char*)" is pulled to");
				cramlc(1, quadx, quady);
				proutn((char*)", ");
				cramlc(2, sectx, secty);
				skip(1);
				if (resting) {
					prout((char*)"(Remainder of rest/repair period cancelled.)");
					resting = 0;
				}
				if (shldup==0) {
					...
					else prout((char*)"(Shields not currently usable.)");
				}
				newqad(0);
			...
			case FBATTAK: /* Commander attacks starbase */
				...
				ipage = 1;
				skip(1);
				proutn((char*)"Lt. Uhura-  \"Captain, the starbase in");
				cramlc(1, batx, baty);
				skip(1);
				prout((char*)"   reports that it is under atttack and that it can");
				proutn((char*)"   hold out only until stardate ");
				cramf(future[FCDBAS],1,1);
				prout((char*)".\"");
				if (resting) {
					skip(1);
					proutn((char*)"Mr. Spock-  \"Captain, shall we cancel the rest period?\"");
					if (ja()) {
						resting = 0;
						Time = 0.0;
						return;
			...
			case FCDBAS: /* Commander succeeds in destroying base */
				...
				if (batx==quadx && baty==quady) {
					...
					newcnd();
					skip(1);
					prout((char*)"Spock-  \"Captain, I believe the starbase has been destroyed.\"");
				}
					...
					ipage = 1;
					skip(1);
					prout((char*)"Lt. Uhura-  \"Captain, Starfleet Command reports that");
					proutn((char*)"   the starbase in");
					cramlc(1, batx, baty);
					prout((char*)" has been destroyed by");
					if (isatb==2) prout((char*)"the Klingon Super-Commander");
					else prout((char*)"a Klingon Commander");
				}
				...
				if (probecx != i || probecy != j) {
					...
					if (i < 1 || i > 8 || j < 1 || j > 8 ||
						...
						if (REPORTS) {
							...
							ipage = 1;
							skip(1);
							proutn((char*)"Lt. Uhura-  \"The deep space probe ");
							if (i < 1 ||i > 8 || j < 1 || j > 8)
								proutn((char*)"has left the galaxy");
							else
								proutn((char*)"is no longer transmitting");
							prout((char*)".\"");
						}
						future[FDSPROB] = 1e30;
						break;
					...
					if (REPORTS) {
						...
						ipage = 1;
						skip(1);
						proutn((char*)"Lt. Uhura-  \"The deep space probe is now in ");
...
void nova(int ix, int iy) {
			...
			for (nn = 1; nn <= 3; nn++)  /* nn,j represents coordinates around current */
				for (j = 1; j <= 3; j++) {
					...
					switch (iquad) {
						...
						case IHF:
							prout((char*)"***Starship buffeted by nova.");
							if (shldup) {
								if (shield >= 2000.0) shield -= 2000.0;
									...
									shield = 0.0;
									shldup = 0;
									prout((char*)"***Shields knocked out.");
	...
	Time = 10.0*dist/16.0;
	skip(1);
	prout((char*)"Force of nova displaces starship.");
...
void snova(int insx, int insy) {
	...
	if (insy== 0) {
		...
		if (nqx != quady || nqy != quady || justin != 0) {
			...
			if (REPORTS) {
				skip(1);
				proutn((char*)"Message from Starfleet Command       Stardate ");
				cramf(d.date, 0, 1);
				skip(1);
				proutn((char*)"     Supernova in");
				cramlc(1, nqx, nqy);
				prout((char*)"; caution advised.");
			}
		}
		else {
	...
	if (insipient) {
		...
		skip(1);
		proutn((char*)"***Incipient supernova detected at");
		cramlc(2, nsx, nsy);
		skip(1);
		nqx = quadx;
		nqy = quady;
	...
	if (d.remkl==0 && (nqx != quadx || nqy != quady)) {
		skip(2);
		if (insx == 0) prout((char*)"Lucky you!");
		proutn((char*)"A supernova in");
		cramlc(1, nqx, nqy);
		prout((char*)" has just destroyed the last Klingons.");
		finish(FWON);
		return;
	}
